...
function drawTree(data, container) {
    ...
    const height = 400;
    const svg = d3.select(container).append("svg").attr("width", "100%").attr("height", height).attr("viewBox", `0 0 ${width} ${height}`).style("background", "#fff").style("border-radius", "12px");
    ...
    nodes.append("text").text(d => d.label).attr("dy", 50).attr("text-anchor", "middle").style("font-size", "12px");
    nodes.append("text").text(d => d.pv ? d.pv.toLocaleString() : "").attr("dy", 65).attr("text-anchor", "middle").style("font-size", "11px").style("fill", "#666");
}
